...
static void
reCreateGraph(const int num_edges, const double mu, const double c) 
{
    ...
    for (i=0; i<numArcs; ++i)
    {
        ...
        ac->capacity = initEdgeList[i].capacity + mu*initEdgeList[i].capacity2;
...
readDimacsFileCreateList (const char *filename) 
{
	...
	while (fgets (line, lineLength, file))
	{
		...
		case 'a':
            ...
            tmpline = getNextWord (tmpline, word);
            //printf ("Word %s\n", word);
            initEdgeList[first].capacity2 = (double) atof (word);
            //printf ("Double %.3f\n", (double) atof (word));
...
extern double c_getMinCutValue(){
    ...
    //printf ("mincut value %.3f:\n", cutValue);
    return cutValue;
}

extern void c_getMinCutEdgeSet(const int num_edges, int *sourceSet)
{
    //int sourceSet[num_edges];
    int i = 0;
    //printf ("Maximal source set nodes&?:\n");
    for (i = 0; i < numNodes; ++i)
    {
...
{
    //int sourceSet[numNodes];    
    int i;    
    *F_edges = 0;
    *F_nodes = 0;
    for (i = 0; i < numNodes; ++i)
    {
        //printf ("%d %d %d\n", adjacencyList[i].number, adjacencyList[i].label, numNodes);
        if (adjacencyList[i].label < numNodes)
        {
            if (adjacencyList[i].number <= num_edges)
            {
...
//extern void c_displayBreakpoints(){displayBreakpoints();}
